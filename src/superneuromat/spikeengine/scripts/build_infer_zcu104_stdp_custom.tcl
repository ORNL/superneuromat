...
# PARAMETRIC ZCU104 STDP build (2026-07-29). Wide fixed-point (W16/D24) + URAM
# synapse packing (SNM_SYN_MEM_ULTRA), reads N_MAX / NUM_LANES /
# SYN_CAP_PER_LANE / SPIKE_MON_BASE / SYNTH_ONLY from -tclargs. Local build path
# + copy-back. Board recipe from build_infer_zcu104_stdp_maxcap1024x16.tcl
# (maxThreads 1, replication on cfg_* nets, route Explore, URAM). Same top-fix
# note as sp701: WEIGHT_W/DATA_W/SPIKE_MON_BASE/SYN_CAP_PER_LANE now actually
# declared+forwarded (were silently dropped before -> 8-bit build).
#
#   vivado -mode batch -source build_infer_zcu104_stdp_custom.tcl \
...
set N_MAX     [expr {[llength $argv] >= 1 ? [lindex $argv 0] : 1024}]
set NUM_LANES [expr {[llength $argv] >= 2 ? [lindex $argv 1] : 16}]
...
puts "CUSTOM_BUILD board=zcu104 tag=$tag N_MAX=$N_MAX NUM_LANES=$NUM_LANES SYN_CAP_PER_LANE=$SYN_CAP LOCAL_D=$LOCAL_D synth_only=$SYNTH_ONLY"
...
set local_outdir [file normalize $build_root/zcu104_custom_$tag]
file mkdir $local_outdir
set final_outdir [file normalize $build_root/results/zcu104_custom_$tag]
file mkdir $final_outdir
set outdir $local_outdir
set bit_name "snm_infer_zcu104_stdp_${tag}.bit"
...
set rtl_files {
    ...
    rtl/fpga/snm_infer_fpga_top_stdp.v rtl/fpga/snm_infer_zcu104_top_stdp.v
}
...
if {$SYNTH_ONLY} {
    ...
    puts "SYNTH_CHECK board=zcu104 N=$N_MAX K=$NUM_LANES CAP=$SYN_CAP => brams=$brams urams=$urams lutcells=$luts ffs=$ffs"
    return
}
...
set brams [llength [get_cells -hier -filter {REF_NAME =~ RAMB*}]]
set urams [llength [get_cells -hier -filter {REF_NAME =~ URAM*}]]
puts "TIMING board=zcu104_$tag part=$part wns=$wns ns luts=$luts ffs=$ffs brams=$brams urams=$urams"
write_bitstream -force $outdir/$bit_name
foreach f [glob -nocomplain $outdir/*] { file copy -force $f $final_outdir }
puts "BUILD_DONE board=zcu104_$tag part=$part bitstream=$final_outdir/$bit_name outdir=$final_outdir"
