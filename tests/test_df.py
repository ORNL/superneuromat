...
class ToNetworkxTest(unittest.TestCase):
    """ Test networkx
    ...
    def test_networkx(self):
        ...
        # test that the function imports networkx
        G = snn.to_networkx()
        import networkx as nx
        import tempfile
        nx.write_graphml(G, tempfile.TemporaryFile())
        # nx.write_graphml(G, 'example_with_attributes.graphml')
        G = snn.to_networkx(include_attributes=False)
        nx.write_graphml(G, tempfile.TemporaryFile())
...
class FromNetworkxTest(unittest.TestCase):
    """ Test networkx

    """

    def test_snn_from_networkx(self):
        snn = SNN()
        ...
        # test that the function imports networkx
        G = snn.to_networkx(include_attributes=True, include_synapse_order=True)
        snn2 = SNN.from_networkx(G)
        # self.assertEqual(snn2.num_neurons, snn.num_neurons)
        # self.assertEqual(snn2.num_synapses, snn.num_synapses)
        print(snn)
        print(snn2)
        assert snn == snn2
